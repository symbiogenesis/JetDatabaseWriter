...
// W17a probe: Jet3 (.mdb Access 97) index TDEF + leaf-page layouts. The W17
// limitation in docs/design/index-and-relationship-format-notes.md says Jet3
// rejects IndexDefinition entirely; W17a establishes empirical ground truth
...
// W6 probe: Jet3 + Jet4 .mdb + ACCDB catalog scan. The W6 phase in
...
static async Task WriteMdbCatalogAppendixAsync(string fixturesDir, string outPath)
{
    ...
    sb.AppendLine($"Generated by: `JetDatabaseWriter.FormatProbe` on {DateTimeOffset.UtcNow:u}");
    sb.AppendLine();
    sb.AppendLine("This appendix grounds the W6 question in [`index-and-relationship-format-notes.md`](index-and-relationship-format-notes.md) §6:");
    ...
    {
        sb.AppendLine("**Verdict.** No fixture in the corpus — across every Access format and version we test against — contains either index-catalog table. Index metadata lives entirely in the per-table TDEF block (sections §3.1 / §3.2) for every JET format this library writes. W6 has no work item.");
    }
    else
    {
        sb.AppendLine("**Verdict.** At least one fixture contains an index-catalog table. The detailed catalogs for those fixtures are reproduced below — W6 must populate the matching system table when writing for that format/version.");
    }
    sb.AppendLine();
...
static async Task WriteJet3IndexAppendixAsync(string fixturesDir, string outPath)
{
    ...
    // to carry indexes. These ground W17a — see
    // docs/design/index-and-relationship-format-notes.md §7 W17 row.
    string[] candidates =
    [
    ...
    var sb = new StringBuilder();
    sb.AppendLine("# Format probe appendix: Jet3 index TDEF + leaf-page layouts (W17a)");
    sb.AppendLine();
    sb.AppendLine($"Generated by: `JetDatabaseWriter.FormatProbe` on {DateTimeOffset.UtcNow:u}");
    sb.AppendLine();
    sb.AppendLine("This appendix grounds the W17 phase in [`index-and-relationship-format-notes.md`](index-and-relationship-format-notes.md):");
    ...
    sb.AppendLine();
    sb.AppendLine("Each fixture below is dumped TDEF-by-TDEF for the user tables that carry at least one logical index, plus a hex dump of the first index leaf page reachable from each real-idx `first_dp`. Compare against the §3.1 / §3.2 / §4.2 mappings; mismatches indicate the spec needs amending before W17b can ship safely.");
    sb.AppendLine();
...
// W17a leaf-page dumper. For every real-idx in the supplied Jet3 TDEF, locates
...
static async Task EmitJet3LeafPagesAsync(AccessReader reader, StringBuilder sb, byte[] td, int numRealIdx)
{
    ...
    sb.AppendLine("### W17a leaf-page resolution probe");
    sb.AppendLine();
    ...
    if (includeIndexAnnotations && numRealIdx > 0)
    {
        ...
        for (int i = 0; i < numRealIdx; i++)
        {
            ...
            else
            {
                ...
                // for. The exact layout of those last 5 bytes is what W17a is trying
                // to pin down.
                sb.AppendLine($"- bytes  0..3  unknown(4):  `{Hex(bytes, start + 0, 4)}`");
                sb.AppendLine($"- bytes  4..33 col_map (10 × {{col_num(2),col_order(1)}}):");
                for (int slot = 0; slot < 10; slot++)
        ...
        if (numIdx > 0)
        {
            ...
            for (int i = 0; i < numIdx; i++)
            {
                ...
                else
                {
                    ...
                    // likely Jet3 mapping (see W17a notes) is:
                    //
