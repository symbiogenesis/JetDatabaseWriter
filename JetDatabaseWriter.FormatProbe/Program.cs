...
Console.WriteLine($"Done. Check the generated markdown files under {designDir} for the probe results.");
return 0;
...
static async Task WriteIndexAppendixAsync(string fixturePath, string outPath)
{
    ...
    _ = sb.AppendLine("It exists to ground the offset claims in [`index-and-relationship-format-notes.md`](index-and-relationship-format-notes.md) against real Access-produced bytes rather than the partial mdbtools spec.");
    _ = sb.AppendLine();
...
static async Task WriteComplexAppendixAsync(string fixturePath, string outPath)
{
    ...
    _ = sb.AppendLine(CultureInfo.InvariantCulture, $"Format: {reader.DatabaseFormat}, page size: {reader.PageSize}");
    _ = sb.AppendLine();
    _ = sb.AppendLine("Regenerate with `dotnet run --project JetDatabaseWriter.FormatProbe`. See [`complex-columns-format-notes.md`](complex-columns-format-notes.md) for the unverified spec this appendix grounds.");
    _ = sb.AppendLine();
...
static async Task WriteMdbCatalogAppendixAsync(string fixturesDir, string outPath)
{
    ...
    _ = sb.AppendLine(CultureInfo.InvariantCulture, $"Generated by: `JetDatabaseWriter.FormatProbe` on {DateTimeOffset.UtcNow:u}");
    _ = sb.AppendLine();
    _ = sb.AppendLine("This appendix grounds the catalog probe question in [`index-and-relationship-format-notes.md`](index-and-relationship-format-notes.md) §6:");
    ...
    foreach (string fixturePath in fixturePaths)
    {
        ...
        try
        {
            ...
            foreach (var c in catalog.Where(c => c.Name.Contains("index", StringComparison.OrdinalIgnoreCase)))
            {
