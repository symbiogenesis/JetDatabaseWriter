...
- **Incremental B-tree maintenance with append-only fast path.** Three fast paths skip the full table-snapshot rebuild on insert / update / delete:
   ...
   3. **Multi-level rebuild from existing tree (W4 sub-phase D).** When the root is a multi-level tree (one or more `0x03` intermediate levels) and the append-only path does not apply, descend to the leftmost leaf, walk the leaf-sibling chain to collect every entry, splice the change-set in, and rebuild a fresh tree from the existing index pages — propagating leaf splits and merges through any number of intermediate levels.

  All paths bail to the bulk path on `T_NUMERIC` keys (canonical scale needs a full snapshot), encoder-rejected key types, or a malformed page chain. There is still **no truly surgical in-place mutation against a multi-level tree** when the change is not an append (the multi-level path always re-emits the whole index from a fresh page-number range, leaving the old pages orphaned for Compact & Repair to reclaim). Emitted leaf and intermediate pages use shared-prefix compression (`pref_len > 0`).
