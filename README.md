...
### Index keys
- **Long index keys are truncated at the upstream cap.** The text/memo encoder mirrors Jackcess's long-standing `TODO long rows not handled completely yet in V2010` limitation: encoded text-key prefixes are capped at 127 chars, and on-disk leaf entries appear to truncate around 508 bytes. The V2010 long-row stress fixtures (`Table11`, `Table11_desc` in `testIndexCodes*.accdb`) and binary single-column long-key fixtures are explicitly skipped by [GeneralLegacyEncoderFixtureTests.cs](JetDatabaseWriter.Tests/Internal/GeneralLegacyEncoderFixtureTests.cs) and [NonTextSingleColumnIndexFixtureTests.cs](JetDatabaseWriter.Tests/Internal/NonTextSingleColumnIndexFixtureTests.cs); the [IndexCodesAggregateDiagnosticTests.cs](JetDatabaseWriter.Tests/Internal/IndexCodesAggregateDiagnosticTests.cs) aggregate also ignores Memo-keyed indexes for the same reason. Canonical tracking: [docs/design/test-coverage-gaps.md](docs/design/test-coverage-gaps.md) §1.1.
