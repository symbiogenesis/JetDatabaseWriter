...
### Indexes
- **Jet3 (`.mdb` Access 97) live leaf maintenance is at full parity with Jet4 / ACE.** The writer emits the §3.1 39-byte real-idx descriptor, the §3.2 20-byte logical-idx entry, and a Jet3 leaf page (page size 2048, bitmask at `0x16`, first entry at `0xF8`). Subsequent insert/update/delete calls run through the same `IndexLeafPageBuilder.LeafPageLayout` descriptor as Jet4/ACE, including the W4-C single-leaf splice and W4 sub-phase D multi-level rebuild fast paths (W17d, 2026-04-26) — so Jet3 mutations are no longer always O(N) per write. The W15 pre-write unique-index check participates on Jet3 too via the per-format §3.1 / §3.2 offsets.
- **Indexable key types cover every scalar JET column type Microsoft Access itself permits to index.** Supported key types: `BYTE`, `INT`, `LONG`, `MONEY`, `FLOAT`, `DOUBLE`, `DATETIME`, `GUID`, `NUMERIC` (decimal-backed), `BINARY` (W19, 2026-04-26 — variable raw bytes via the same Jackcess "general binary entry" packing as `GUID`), plus `TEXT` / `MEMO` across the entire BMP via a port of the Jackcess "General Legacy" sort-key encoder (Apache 2.0; see `THIRD-PARTY-NOTICES.md`). `NUMERIC` participates in every fast path (W4-C / W4-C-3 / W4-C-4 / W18 / W4-D) using the column's DECLARED scale (W23, 2026-04-27 — `ColumnDefinition.NumericPrecision` / `NumericScale`, persisted at TDEF column-descriptor offsets 11/12, default `18`/`0` matching the Access "Number → Decimal" UI defaults); values whose natural scale exceeds the declared scale are rounded half-to-even just as Access does on store. Indexes over `OLE Object`, `Attachment`, and `Multi-Value (Complex)` columns are not listed here because Microsoft Access itself does not permit them; the writer rejects such `IndexDefinition`s with `NotSupportedException`.
- **Incremental B-tree maintenance with append-only fast path.** Four fast paths skip the full table-snapshot rebuild on insert / update / delete:
   1. **Single-leaf splice (W4-C).** When the index B-tree is rooted on a single leaf page and the post-mutation entry list still fits on one page, the change is spliced into the leaf in place.
   2. **Append-only tail rewrite (W18, 2026-04-26).** When the change-set is insert-only AND every new key sorts strictly after the current tail-leaf max, the tail leaf is rewritten in place — no descend, no walk, no rebuild. The bulk builder stamps each intermediate page's `tail_page` header field with the rightmost-leaf page number (§4.5) and `IndexBTreeSeeker` follows it on overshoot, so the rightmost intermediate's stale summary entry does not break parent-PK seeks.
   3. **Surgical multi-level mutation (W4-C-3 / W4-C-4, 2026-04-27).** When every change in the batch lands on the SAME leaf, the change is mutated in place at the affected leaf's existing page number — preserving its `prev_page` / `next_page` sibling pointers and re-emitting just the parent intermediate (and only further ancestors when the leaf's max key was the last summary on that intermediate) if its summary key changed. If the spliced entry list no longer fits one page, the leaf is split 2-way: one new leaf is appended for the right half, the original leaf is rewritten in place as the left half, the next-sibling leaf's `prev_page` is patched, and one new summary entry is inserted into the parent intermediate in place. No orphaned pages on success.
   4. **Multi-level rebuild from existing tree (W4 sub-phase D).** When none of the above apply (multi-leaf change-sets, leaf-becomes-empty underflow, 3+ page splits, parent intermediate overflow on summary insert, key types the encoder cannot accept), descend to the leftmost leaf, walk the leaf-sibling chain to collect every entry, splice the change-set in, and rebuild a fresh tree from the existing index pages — propagating leaf splits and merges through any number of intermediate levels.

  All paths bail to the bulk path on encoder-rejected key types or a malformed page chain. Files written before W23 (2026-04-27) — when the descriptor's NUMERIC precision/scale bytes were emitted as zero — fall back to the bulk pre-pass on every NUMERIC mutation; once such a column is rewritten by a CREATE TABLE under W23+ the declared scale is persisted and the incremental paths engage. **Surgical in-place mutation now covers most non-append non-empty mutations against multi-level trees** (W4-C-3 / W4-C-4, 2026-04-27 — same-leaf change-sets including 2-way leaf splits propagate through one level of intermediate updates). Recursive intermediate split, leaf merge / redistribute on delete underflow, and changes spanning multiple leaves still fall back to the W4-D rebuild (which leaves the old pages orphaned for Compact & Repair to reclaim). Emitted leaf and intermediate pages use shared-prefix compression (`pref_len > 0`).
