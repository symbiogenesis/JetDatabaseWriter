...
| ACCDB Agile | Access 2010 SP1+, Microsoft 365 (`.accdb`) | Access-native flat Agile header: masked encoding key at page-0 `0x3E`, `EncryptionInfo` length at `0x299`, descriptor at `0x29B`; CFB Office Crypto Agile wrappers are readable, but writer output uses the flat Access-native layout | ECMA-376 §2.3.4.11 PBKDF: SHA-512 + `spinCount` iterations + spec block keys (`0xfea7d2763b4b9e79`, `0xd7aa0f6d3061344e`, `0x146e0be7abacd0d6`) | AES-256-CBC per data page; IV `SHA-512(keyDataSalt || (uint32_le(pageNumber) XOR encodingKey))[:16]` |
...
### Index keys
- **Long index keys with embedded line breaks are now encoded byte-exact (Jet 4) / 508-of-510 bytes matched (V2010 ACE).** Previously the text/memo encoder mirrored Jackcess's long-standing `TODO long rows not handled completely yet in V2010` limitation: encoded prefixes were capped at 127 chars and any value beyond that was silently dropped. For Jet 4 (V2000 / V2003 / V2007), the encoder now emits the 2-chunk long-row layout reverse-engineered from the Access-authored `Table11` / `Table11_desc` fixtures; these are validated byte-exact in [GeneralLegacyEncoderFixtureTests.cs](JetDatabaseWriter.Tests/Indexes/Collation/GeneralLegacyEncoderFixtureTests.cs). For V2010 (ACE), Access uses a different layout: continuous 255-char encoding hard-truncated to 510 bytes. The encoder matches the first 508 bytes; the final 2-byte suffix is an unknown algorithm (exhaustive testing of ~3.4 M hash/CRC/input combinations found no match). Open: V2010 2-byte suffix derivation, and the no-line-break long-input branch (no Access-authored fixture). See [docs/format-probe/format-probe-long-row-index-encoding.md](docs/format-probe/format-probe-long-row-index-encoding.md) and [docs/test-coverage-gaps.md](docs/test-coverage-gaps.md) §1.1.

### Round-trip through Microsoft Access Compact & Repair
- **Covered writer paths are validated against DAO Compact & Repair on Access-equipped Windows hosts.** The suite includes writer-created tables, indexes, primary and foreign keys, DAO relationship enforcement, FK Compact & Repair, and password-protected ACCDB compact/reopen. Historical Compact & Repair failures and their fixes are documented in [docs/design/round-trip-test-failures.md](docs/design/round-trip-test-failures.md) and [docs/design/round-trip-openrecordset-hypothesis.md](docs/design/round-trip-openrecordset-hypothesis.md).
