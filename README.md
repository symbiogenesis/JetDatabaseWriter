...
### Indexes
- **Jet3 (`.mdb` Access 97) indexes are schema-only.** `IndexDefinition` lists are accepted on Jet3 — the writer emits the §3.1 39-byte real-idx descriptor, the §3.2 20-byte logical-idx entry, and an empty Jet3 leaf page (page size 2048, bitmask at `0x16`, first entry at `0xF8`). However, **live leaf maintenance is not implemented for Jet3**: subsequent insert/update/delete leaves the leaf empty until Microsoft Access rebuilds it on the next Compact & Repair pass. Same fallback model already used for unsupported Jet4 key types.
