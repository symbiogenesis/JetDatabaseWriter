...
- **Indexable key types are limited.** Some scalar JET column types (notably `NUMERIC` / decimal-backed and `BINARY`) are not yet wired through `IndexKeyEncoder`; indexes whose key columns include them fall through to the schema-only path and Access rebuilds the leaf on Compact & Repair. If *any* column in a multi-column index is unsupported, the whole index falls through to the schema-only path. Supported key types today: `BYTE`, `INT`, `LONG`, `MONEY`, `FLOAT`, `DOUBLE`, `DATETIME`, `GUID`, plus `TEXT` / `MEMO` across the entire BMP via a port of the Jackcess "General Legacy" sort-key encoder (Apache 2.0; see `THIRD-PARTY-NOTICES.md`). Indexes over `OLE Object`, `Attachment`, and `Multi-Value (Complex)` columns are not listed here because Microsoft Access itself does not permit them; the writer rejects such `IndexDefinition`s with `NotSupportedException`.
