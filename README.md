...
### Indexes
- **Jet3 (`.mdb` Access 97) live leaf maintenance is supported.** The writer emits the §3.1 39-byte real-idx descriptor, the §3.2 20-byte logical-idx entry, and a Jet3 leaf page (page size 2048, bitmask at `0x16`, first entry at `0xF8`). Subsequent insert/update/delete calls run the same bulk B-tree rebuild path as Jet4/ACE through the per-format `IndexLeafPageBuilder.LeafPageLayout` descriptor. The W4 incremental fast paths (single-leaf splice + multi-level rebuild) are still Jet4-only — Jet3 mutations always go through the bulk-rebuild path, which is functionally correct but O(N) per write.
...
- **Partial incremental B-tree maintenance.** Two fast paths skip the full table-snapshot rebuild on insert / update / delete: (1) when the index B-tree is rooted on a single leaf page and the post-mutation entry list still fits on one page, the change is spliced into the leaf in place (W4-C-1 / W4-C-2); (2) when the root is a multi-level tree (one or more `0x03` intermediate levels), we descend to the leftmost leaf, walk the leaf-sibling chain to collect every entry, splice the change-set in, and rebuild a fresh tree from the existing index pages — propagating leaf splits and merges through any number of intermediate levels (W4 sub-phase D). Both paths bail to the bulk path on `T_NUMERIC` keys (canonical scale needs a full snapshot), encoder-rejected key types, or a malformed page chain. There is still no `tail_page` append optimisation and no truly surgical in-place mutation against a multi-level tree (the multi-level path always re-emits the whole index). Emitted leaf and intermediate pages use shared-prefix compression (`pref_len > 0`).
