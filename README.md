...
- **Index creation: schema + bulk leaf rebuild on every mutation ("schema-W1 + W2/W3 + W5").** `CreateTableAsync` accepts an optional `IReadOnlyList<IndexDefinition>` that declares logical indexes on the new table.
  ...
  - **Supported `IndexDefinition` shape.** Single-column, non-unique, ascending only. Multi-column indexes, unique indexes, descending keys, primary keys, foreign keys, relationships, and cascade flags are not supported. Jet3 (`.mdb` Access 97) databases reject `IndexDefinition` entirely.
  - **Supported key column types.** `Byte`, `Integer`, `Long Integer`, `Currency`, `Single`, `Double`, `Date/Time`. Indexes declared on `Text`, `GUID`, `Decimal`, `OLE`, `MEMO`, attachment, and complex columns silently skip B-tree rebuild on mutation (the on-disk leaf goes stale until Microsoft Access rebuilds it on Compact & Repair); the structural metadata still round-trips through `ListIndexesAsync`.
  - **Index forwarding through schema evolution.** `AddColumnAsync` preserves all surviving indexes. `DropColumnAsync` drops indexes whose key column was removed. `RenameColumnAsync` rewrites the index column reference to the new name.
  - **Validation gap.** Files produced with non-empty `IndexDefinition` lists have not yet been opened in Microsoft Access on Windows for a Compact & Repair smoke test. Defer production use until that round-trip has been verified by hand.
