...
**Status:** C1 + C2 + C3 implemented (2026-04-25). C4–C6 pending.
**Related limitations:** [`README.md` §"Limitations"](../../README.md):
- "Attachment & multi-value (complex) columns — schema only. `CreateTableAsync` accepts the declarations and emits the hidden flat child table + `MSysComplexColumns` row; row-level Insert/Add APIs (C4), cascade-on-delete (C5), and `DropTableAsync` cascade (C6) are still pending."
