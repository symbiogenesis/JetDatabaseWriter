...
**Status:** Phase C0 + C1 shipped. A **prefix compression cap fix (2026-05-03)** and an **entry-start bitmask sentinel fix (2026-05-04)** have resolved the N1 (single-table) reproducer — DAO now accepts the writer's output when creating a single table. The N2 (two-table) case still fails with `Object invalid or no longer set` because the split path in `TryBuildSplitLeafPages` does not pass `maxPrefixLength` to `BuildLeafPage` — when the second splice overflows page 2790 and triggers an N-way split, the split products get unrestricted prefix compression (pref_len grows from 1→4 on rewritten page 2790, 16 on new page 3019), causing DAO rejection. See [`round-trip-test-failures.md`](round-trip-test-failures.md) §"N2 failure analysis" for the full investigation.
...
Both currently fail with DAO errors. Post-sentinel-fix: N1 (single `CreateTableAsync`) passes DAO compact, but N2+ (two tables + relationship) still fails with `Object invalid or no longer set`. Binary page-level bisection isolated the N1 cause to the splice itself; the sentinel fix resolved it.
...
- **N2 (two tables):** Still fails with `Object invalid or no longer set`. The second `CreateTableAsync` call's splice overflows page 2790 (free=10 < entry_size≈24), triggering `TryGreedySplitLeafInN`. The split path (`TryBuildSplitLeafPages`) does not pass `maxPrefixLength`, so the split products get pref=4 (page 2790) and pref=16 (new page 3019) instead of ≤1. DAO rejects pages whose pref_len exceeds the original author's value.
...
| **C1** | `IndexMaintainer.TrySpliceCatalogIndexEntryAsync` wired into `AccessWriter.InsertCatalogEntryAsync`; tail-leaf append for monotonic Id inserts. | **Shipped.** Splice verified byte-correct against both MSysObjects real-idx slots. Prefix compression cap fix (2026-05-03) + bitmask sentinel fix (2026-05-04) landed. **N1 (single table) now passes DAO compact.** N2+ gating tests still fail — `TryBuildSplitLeafPages` omits the `maxPrefixLength` cap on split-product pages (see `round-trip-test-failures.md` §"N2 failure analysis"). |
...
**C0 + C1 have shipped.** C2/C3/C4 reduce technical debt but are not gating. The N1 reproducer passes DAO compact; the remaining N2+ failure is tracked in [`round-trip-test-failures.md`](round-trip-test-failures.md) — `TryBuildSplitLeafPages` omits the `maxPrefixLength` cap on split-product pages, allowing `pref_len` to grow beyond the original page's value after a leaf split.
