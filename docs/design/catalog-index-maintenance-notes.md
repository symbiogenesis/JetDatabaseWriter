...
Phase C0 + C1 (below) closed that path: every `InsertCatalogEntryAsync` call now splices the new row's keys into every real-idx leaf of MSysObjects. The current gating-test failure is a separate writer defect on the new TDEF page itself — see [`round-trip-test-failures-2026-05-02.md`](round-trip-test-failures-2026-05-02.md).
...
4. **Descend the B-tree** to the target leaf via `IndexBTreeSeeker` descent logic. Big-endian intermediate child pointers (see top-of-doc "do not regress" list in [`round-trip-test-failures-2026-05-02.md`](round-trip-test-failures-2026-05-02.md)).
...
**C0 + C1 have shipped.** C2/C3/C4 reduce technical debt but are not gating. The remaining gating-test failure is tracked in [`round-trip-test-failures-2026-05-02.md`](round-trip-test-failures-2026-05-02.md) (new-table TDEF `ump_page` zero).
...
The two gating round-trip tests (§1) **do not yet pass**, but they fail on the new-table TDEF defect described in [`round-trip-test-failures-2026-05-02.md`](round-trip-test-failures-2026-05-02.md), not on the splice.
