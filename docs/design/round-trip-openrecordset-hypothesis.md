...
| CFB Agile wrapper for Access-native password databases | Hypothesis: modern Access/DAO encrypted `.accdb` output is a CFB Office Crypto package. | Disconfirmed for DAO compact compatibility. DAO-created password-protected ACCDBs use flat Access-native Agile encryption, not a CFB wrapper. The library still understands Office Crypto CFB containers, but `AccessEncryptionFormat.AccdbAgile` now writes the flat Access-native layout. |
