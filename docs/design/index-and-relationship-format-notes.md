...
- **W10 caveats:** insert-side enforcement is now an O(log N) parent-index seek (W16) on Jet4/ACE; cascade-update / cascade-delete on the parent still load an O(N) child snapshot per relationship per call. No SET NULL / SET DEFAULT cascade actions; no transaction or rollback; no FK enforcement on `RewriteTableAsync`.
...
Replaces the W10 O(N) parent-table snapshot inside `EnforceFkOnInsertAsync` with an O(log N) B-tree seek over the parent's PK / FK index. Cascade-update / cascade-delete on the parent (`EnforceFkOnPrimaryUpdateAsync`, `EnforceFkOnPrimaryDeleteAsync`) still load a child snapshot — those paths need a reverse FK-side seek surface that W16 does not yet add.
