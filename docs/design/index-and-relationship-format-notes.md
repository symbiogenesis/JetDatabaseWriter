...
- **Recursive ancestor empties still bail.** If, in a multi-group batch, two leaves under the same parent both empty out and the parent would lose all its entries, the existing `if (newEntries.Count == 0)` bail in `TryStageIntermediateRewrites` triggers and the batch falls to W4-D. Recursive intermediate-collapse is W4-C-8+ scope.
...
- **3+ page splits at any captured level still bail.** `TryGreedySplitIntermediateInTwo` still refuses inputs whose entries cannot be packed into exactly two pages. Closing this is W4-C-8.
...
- **W4 sub-phase C — incremental B-tree maintenance.** 🟡 **Mostly shipped.** Sub-phases shipped to date: **W4-C-1 / W4-C-2** (single-leaf incremental INSERT / DELETE — §7.9, 2026-04-26), **W4-C-3 / W4-C-4** (single-leaf surgical multi-level descent + 2-way leaf split with parent-of-leaf summary update — §7.12, 2026-04-27), **W4-C-5** (cross-leaf change-set support via per-leaf grouping in `TrySurgicalCrossLeafMaintainAsync`, 2026-04-27), **W4-C-6 v1** (leaf-merge on delete underflow when the dead leaf is NOT the parent's rightmost child, 2026-04-27), **W4-C-7 v1** (intermediate split + grandparent absorb / root reallocation when the splitting page IS the parent-of-leaf, 2026-04-27), **W4-C-6 v2** (leaf-merge on the rightmost-child case via `tail_page` recomputation propagated up every captured ancestor — §7.14, 2026-04-27), and **W4-C-7 v2** (recursive intermediate split through arbitrary depth via `GetEffectiveTailPageAsync` lookup of staged-or-live child `tail_page` headers, 2026-04-27). Outstanding sub-phases tracked as W4-C-8 below.
- **W4-C-8 — N-way leaf split when greedy 2-way overflows.** `TryGreedySplitInTwo` (and its intermediate-page sibling `TryGreedySplitIntermediateInTwo`) refuses any input whose entries cannot be packed into exactly two pages. Workloads that hit this: a single-leaf bulk insert whose splice runs to ≥ 2.x pages (rare on INT keys, common on long composite TEXT/MEMO keys), or any cross-leaf batch where one per-leaf splice straddles the 2-page boundary. Closing this means generalising both helpers to N-way (greedy left-fill until overflow, allocate next page, repeat) and updating `PrepareAncestorSplitWrites` and the `IntermediateOpType.InsertAfter` op aggregator to insert N-1 new summary entries into the parent intermediate at the original child's slot — which itself is more likely to overflow the parent and re-engage W4-C-7 (likely v2). Probably worth landing AFTER W4-C-7 v2 so the N-way splits have a working recursive intermediate-split safety net to land on. Bail floor: keep a `MaxLeafSplitWidth ≈ 8` cap matching the existing `MaxLeafGroupCount = 64` heuristic. Tests in a new `IndexLimitationsW4C8Tests.cs` would exercise: 3-page split on a single leaf (asserts exactly 2 new index pages appended, parent gets 2 new summaries), 4-page split on the rightmost leaf (asserts `tail_page` cascade still correct on every ancestor), and the bail-floor case (5+ pages → fall through to W4-D).
- **W4-C-8+ — recursive intermediate collapse on cascading underflow.** When a multi-group delete batch empties two leaves whose parents both then become single-entry intermediates, today's path leaves both single-entry intermediates standing (the seeker descends through their lone child pointer, which is correct but suboptimal). When a delete batch would empty a parent intermediate entirely (`newEntries.Count == 0` in `TryStageIntermediateRewrites`), the path bails. Closing this means recognising "intermediate has zero entries → remove its summary from the grandparent → recurse" with the same `tail_page` cascade machinery W4-C-6-v2 already established. Lowest priority of the three; the bail produces a correct (if larger) tree via the W4-D fallback today.
