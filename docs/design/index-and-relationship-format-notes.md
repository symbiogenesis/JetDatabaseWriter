...
This matches the offsets `AccessReader.ParseIndexMetadata` already uses for the Jet3 best-effort path (`realIdxNum @ +8` is a typo in the reader: it co-locates with `index_num2 @ +4`; both point at the same 4-byte slice as `index_num` is implicit on Jet3 from the 0-based slot order — re-verify before W17b ships).
...
| **W17** | Lift the "Jet3 (`.mdb` Access 97) rejects `IndexDefinition` entirely" limitation. Pin the Jet3 real-idx (39 B), logical-idx (20 B), and leaf-page (`0x04`) layouts empirically; emit them from `BuildTDefPageWithIndexOffsets`; route Jet3 leaf builds through `IndexLeafPageBuilder` / `IndexBTreeBuilder` (page size 2048, bitmask at `0x16`, first entry at `0xF8`); thread Jet3 through `MaintainIndexesAsync` and the W15 unique pre-check. | medium | 🟡 **W17a only.** **W17a (probe + spec) shipped (2026-04-26):** [`JetDatabaseWriter.FormatProbe`](../../JetDatabaseWriter.FormatProbe/Program.cs) `WriteJet3IndexAppendixAsync` dumps the TDEF + first leaf page for every user table with `numIdx > 0` across 6 Jet3 fixtures (`indexTestV1997.mdb`, `compIndexTestV1997.mdb`, `testIndexCodesV1997.mdb`, `testV1997.mdb`, `mdbtools/nwind.mdb`, `Jet3Test.mdb`) into [`format-probe-appendix-jet3-index.md`](format-probe-appendix-jet3-index.md). Output confirmed (i) **`first_dp` lives at phys-desc bytes 34..37 on Jet3** — *not* in the leading 8-byte skip-entry block as the mdbtools spec implies for Jet4-style `used_pages`; (ii) `index_type = 0x01` PK discriminator at logical-idx byte 19; (iii) §4.2 leaf bitmask at `0x16` / first entry at `0xF8` with the 9-byte stride for `T_LONG` keys matching the bitmask LSB-first pattern exactly; (iv) §4.1 page header is identical between Jet3 and Jet4. Spec corrections landed in §3.1 ("Jet3 real-idx physical descriptor"), §3.2 ("Jet3 logical-idx entry"), and §4.2 ("Jet3 confirmed empirically"). **Not done:** **W17b** (TDEF emission lifting the `Jet3Mdb` rejection in `AccessWriter.CreateTableAsync`); **W17c** (`IndexLeafPageBuilder` + `IndexBTreeBuilder` Jet3 variants — page size 2048, bitmask at `0x16`, first entry at `0xF8`); **W17d** (round-trip tests mirroring `IndexWriterTests` / `IndexMaintenanceTests` / `PrimaryKeyWriterTests` for Jet3); **W17e** (README "Limitations" wording update). The `LimitationsTests.SchemaEvolution_*` pinned tests still pass. See §7.7 below. |
