...
- **W4 sub-phase C — incremental B-tree maintenance.** 🟡 **Mostly shipped.** Sub-phases shipped to date: **W4-C-1 / W4-C-2** (single-leaf incremental INSERT / DELETE — §7.9, 2026-04-26), **W4-C-3 / W4-C-4** (single-leaf surgical multi-level descent + 2-way leaf split with parent-of-leaf summary update — §7.12, 2026-04-27), **W4-C-5** (cross-leaf change-set support via per-leaf grouping in `TrySurgicalCrossLeafMaintainAsync`, 2026-04-27), **W4-C-6 v1** (leaf-merge on delete underflow when the dead leaf is NOT the parent's rightmost child, 2026-04-27), **W4-C-7 v1** (intermediate split + grandparent absorb / root reallocation when the splitting page IS the parent-of-leaf, 2026-04-27), and **W4-C-6 v2** (leaf-merge on the rightmost-child case via `tail_page` recomputation propagated up every captured ancestor — §7.14, 2026-04-27). Outstanding sub-phases tracked as W4-C-7 v2 and W4-C-8 below.
- **W4-C-7 v2 — recursive intermediate split through arbitrary depth.** When a 2-way leaf split's parent intermediate overflows on summary insert, today's `TryStageIntermediateRewrites` only splits the parent if it is itself a parent-of-leaf (deepest captured intermediate level, which lets the surgical path compute split halves' `tail_page` from the post-mutation `lastChildPage` directly). When the overflowing intermediate is a HIGHER level (children are themselves intermediates), the path bails to W4-D because the split halves' `tail_page` would have to be computed by reading each child intermediate's own `tail_page` header. Closing this requires (a) staging a recursive `(level, leftPageNumber, rightPageNumber, splitSummary)` plan that walks bottom-up until either some ancestor accepts the new summary without overflow OR the root itself splits, (b) reading every child intermediate's `tail_page` to populate the split halves' headers correctly, and (c) handling the "root splits at a level above parent-of-leaf" case (single root reallocation already exists for the parent-of-leaf case). Reference: Jackcess `IndexData.splitIndexPages` recursive walker. Bail floor stays at `MaxRootSplitDepth ≈ 8`. Tests in a new `IndexLimitationsW4C7V2Tests.cs` would exercise: parent-AND-grandparent at capacity (asserts exactly two new intermediate pages appended), root-at-non-parent-of-leaf-level splits (asserts `first_dp` patched), and `tail_page` correctness on every newly-allocated right-spine page after a far-right split.
