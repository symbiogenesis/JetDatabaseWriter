...
- Two existing tests in [`IndexLimitationsW4C6Tests`](../../JetDatabaseWriter.Tests/Core/IndexLimitationsW4C6Tests.cs) flipped from "asserts W4-D bail (page count strictly increases)" to "asserts surgical merge engaged (page count unchanged)":
...
Tests: 3 new round-trip tests in [`IndexLimitationsW4C7v2Tests`](../../JetDatabaseWriter.Tests/Core/IndexLimitationsW4C7v2Tests.cs) exercise multi-level (composite 255-byte TEXT key) trees through bulk insert + cross-leaf delete, drip-feed inserts that cascade splits up through mid-level intermediates, and delete-and-reinsert cycles. The full 2335-test suite passes; manual Access compact-and-repair validation remains pending alongside every other §7 sub-phase (§8 gap).
